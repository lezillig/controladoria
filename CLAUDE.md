...
Testes são scripts `tsx` puros, sem framework e sem banco — cada um imprime `ok`/`FALHA` por caso e sai com código 1 se algo falhar. Rodar um conjunto: `npm run teste:calibragem` (idem `cte`, `acessos`, `antifraude`, `dre`, `engine`, `historico`, `padroes`, `alerta`, `previsao`, `limpeza`, `frota`, `desvios`, `pessoal`, `contratos`, `receita`, `mapeamento`, `fiscal`, `datas`). Não há "rodar um caso só": edite o script ou filtre a saída com `grep`. `teste:sql` e `teste:limpeza` precisam de um Postgres real em `TESTE_DATABASE_URL` e se auto-pulam sem ela (instruções no topo de `scripts/teste-sql.ts`). O GitHub Actions (`.github/workflows/testes.yml`) roda tipos, lint e todos os conjuntos a cada push em `main`.
...
**Pessoal** (`agents/pessoal.ts`, área RH): título a pagar cujo documento é CPF de motorista, cruzado com o rastro operacional da gestão — ponto (`lerPontos`) e afastamento (`lerAfastamentos`, ambos opcionais como os de frota), escala, uso de veículo e abastecimento. Pago a desligado, fantasma, diária fora do padrão, reembolso duplicado, adiantamento aberto, afastado com operação; toda regra cala sem o dado que a sustenta e exige idade mínima do título pelo atraso do ponto (`scripts/teste-pessoal.ts`).
...
**Detalhamento** (`detalhamento.ts` + `/detalhamento`): o último degrau do painel — cada fatia de cartão abre a lista dos registros que a compõem (títulos do mês, baixas com juros/multa/tarifa/desconto, extrato de uma conta). A regra que sustenta o arquivo: **cada consulta repete literalmente o filtro do número que ela abre**, e a tela mostra a soma das linhas ao lado, para um desencontro aparecer em vez de se esconder. O invariante é testado contra Postgres real em `teste:sql`.
...
**IA**: `aiAnalyst.ts` (narrativa do relatório, `claude-fable-5-1`, saída estruturada), `investigador.ts` (perguntas com ferramentas somente-leitura, `claude-sonnet-5`, rodadas persistidas em `Investigacao`; `propor_tratativa` só propõe — a pessoa aplica pela ação `tratarAchado`), `conformidade/analise.ts` (leitura dos relatórios de consultoria, `claude-sonnet-5` com esforço `medium` — é transcrição, não raciocínio; o porquê e o que faria voltar atrás estão no topo do arquivo). Consulte a skill `claude-api` antes de mexer em chamadas ao modelo.
